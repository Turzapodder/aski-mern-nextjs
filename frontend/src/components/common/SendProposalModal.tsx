...
import { useCreateProposalMutation, useUpdateProposalMutation } from '@/lib/services/proposals';
  ...
  useEffect(() => {
    if (proposalToEdit) {
      setFormData({
        ...
        proposedPrice: proposalToEdit.proposedPrice || assignment.budget || assignment.estimatedCost || 0,
        estimatedDeliveryTime: proposalToEdit.estimatedDeliveryTime || 24,
        relevantExperience: proposalToEdit.relevantExperience || '',
      });
    } else {
      setFormData({
        title: '',
    ...
    setAttachments([]);
    setErrors({});
  }, [proposalToEdit, isOpen, assignment]);
    ...
    <div className="fixed inset-0 bg-black/15 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">{proposalToEdit ? 'Edit Proposal' : 'Send Proposal'}</h2>
            <p className="text-gray-600 mt-1">{proposalToEdit ? 'Update your terms for this assignment' : 'Submit your proposal for this assignment'}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-black focus:border-transparent ${errors.title ? 'border-red-500' : 'border-gray-300'
                }`}
          ...
          <div>
            ...
            <textarea
              ...
              rows={4}
              className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-black focus:border-transparent ${errors.description ? 'border-red-500' : 'border-gray-300'
                }`}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-black focus:border-transparent ${errors.proposedPrice ? 'border-red-500' : 'border-gray-300'
                  }`}
                placeholder="0.00"
              />
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-black focus:border-transparent ${errors.estimatedDeliveryTime ? 'border-red-500' : 'border-gray-300'
                  }`}
                placeholder="24"
              />
          ...
          {/* File Upload (Hidden when editing as attachments update is not supported by backend PUT) */}
          {!proposalToEdit && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
                ...
                <p className="text-gray-600 mb-2">
                  ...
                  <label className="text-black hover:underline cursor-pointer">
                    browse
                    <input
                      type="file"
                      multiple
          ...
          <div className="flex items-center justify-end space-x-4 pt-6 border-t">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="px-6 py-3 bg-black hover:bg-gray-900 text-white rounded-lg disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              {isLoading ? 'Saving...' : (proposalToEdit ? 'Save Changes' : 'Send Proposal')}
            </button>
          </div>
        </form>
      </div>
