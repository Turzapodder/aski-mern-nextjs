...
const TutorComponent = () => {
  ...
  const fetchTutors = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}/api/tutors${queryString ? `?${queryString}` : ''}`);
  ...
  const TeacherCard = ({ teacher }: { teacher: Teacher }) => {
    ...
    return (
      <div
        className={`rounded-2xl p-6 relative flex flex-col h-full transition-transform hover:scale-[1.01] duration-200 ${
          isDark
      ...
      >
        <div className="flex items-start justify-between mb-4">
          ...
          <button
            className={`p-1.5 rounded-full transition-colors ${
              isDark
                ? 'hover:bg-gray-800 bg-[#27272a] text-purple-400'
        ...
        <p
          className={`text-[13px] leading-relaxed mb-6 flex-grow ${isDark ? 'text-gray-300' : 'text-gray-600'}`}
        >
          {teacher.bio}
        </p>

        <div
          className={`grid grid-cols-2 gap-y-2 text-xs mb-6 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}
        >
        ...
        <div className="flex items-center shrink-0">
          ...
          >
            <div className="flex items-center gap-3">
              <div
                className={`w-10 h-6 flex items-center rounded-full px-1 duration-300 ease-in-out ${isTeacherAccountActive ? 'bg-purple-600 justify-end' : 'bg-gray-500 justify-start'}`}
              >
              ...
              <div className="w-5 h-5 rounded-full border border-gray-400 flex items-center justify-center text-xs text-gray-500">
                ?
              </div>
            </div>
          </button>
