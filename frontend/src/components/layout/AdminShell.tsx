"use client"
// Client wrapper for the admin dashboard layout.
// Handles auth check (via useAdminAuth), mobile sheet, navigation indicator —
// keeping the parent layout.tsx as a Server Component.
...
export function AdminShell({ children }: { children: React.ReactNode }) {
    ...
    const { activePath, isNavigating, handleNavigate } = useNavigation()
            ...
            {NAV_ITEMS.map((item) => {
                ...
                return (
                    <Link
                        key={item.href}
                        href={item.href}
                        onClick={() => handleNavigate(variant === "mobile" ? () => setMobileOpen(false) : undefined)}
                        className={cn(
