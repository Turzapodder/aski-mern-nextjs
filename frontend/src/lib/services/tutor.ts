...
export const tutorApi = createApi({
  ...
  baseQuery: fetchBaseQuery({ 
    baseUrl: `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/tutor/`,
    credentials: 'include',
  ...
  endpoints: (builder) => ({
    ...
    submitTutorApplication: builder.mutation<TutorResponse, TutorApplicationData>({
      query: (applicationData) => {
        const formData = new FormData();
        
        // Add JSON data as strings
        formData.append('personalInfo', JSON.stringify(applicationData.personalInfo));
        formData.append('academicInfo', JSON.stringify(applicationData.academicInfo));
        formData.append('quizSummary', JSON.stringify(applicationData.quizSummary));
        ...
        if (applicationData.documents?.profilePicture) {
          formData.append('profilePicture', applicationData.documents.profilePicture);
        }
    ...
    getAllTutorApplications: builder.query<TutorResponse, {
      page?: number;
      limit?: number;
      status?: string;
      subject?: string;
    ...
    reviewTutorApplication: builder.mutation<TutorResponse, {
    ...
    // Save quiz result
    saveQuizResult: builder.mutation<TutorResponse, {
      subject: string;
      topics: string[];
      totalQuestions: number;
      correctAnswers: number;
    ...
    // Get quiz history
    getQuizHistory: builder.query<TutorResponse, {
      page?: number;
      limit?: number;
      subject?: string;
      quizType?: string;
