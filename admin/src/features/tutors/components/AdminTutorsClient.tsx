...
import { TutorRow, statusTone, useAdminTutorsLogic } from '../hooks/useAdminTutorsLogic';
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="mt-3">
          <AdminSectionNav
            items={[
              { label: 'Active tutors', href: '/tutors' },
              { label: 'Verification queue', href: '/tutors/verification' },
            ]}
          />
        </div>
      </div>
                    ...
                    {paginatedTutors.map((tutor: TutorRow) => (
                      <tr key={tutor._id} className="hover:bg-gray-50/60 transition-colors">
                        <td className="py-4 pr-4">
                          <button
                            onClick={() => router.push(`/users/${tutor._id}`)}
